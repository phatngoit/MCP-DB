...
- Fixed `db_list_tables` on Oracle connections silently omitting views: it only queried `all_tables`, unlike every other SQL connector, whose `information_schema`/`sqlite_schema` queries include views alongside tables. Now unions in `all_views`.
- Added pagination to `db_mongo_find` (new `skip` argument) and `db_qdrant_scroll` (new `offset` argument, and a `Next offset` line in the response when more points remain — pass it back as `offset` to continue). Previously `db_qdrant_scroll` could only ever return the first page of a collection since Qdrant's `next_page_offset` was discarded.
- Added `db_mongo_insert`, `db_mongo_update`, and `db_mongo_delete`. Previously setting `mode: readwrite` + `security.allowWriteOperations: true` on a MongoDB connection did nothing useful, unlike SQL connectors where the same flags unlock writes through the existing `db_query` tool; these tools give MongoDB connections an equivalent write path, gated by a new `assertWriteAllowed` guard. `db_mongo_update`/`db_mongo_delete` also require a non-empty `filter` (new `assertNonEmptyFilter` guard) so an accidental `{}` can't update or delete an entire collection.
- Added column comments to `db_describe_table` for Oracle (`all_col_comments`), PostgreSQL (`col_description()`), MySQL/MariaDB (`information_schema.columns.COLUMN_COMMENT`), and MSSQL (`sys.extended_properties` `MS_Description`). The `comment` column only appears in the output when at least one column actually has one; SQLite has no comment mechanism and is unaffected. Verified the PostgreSQL query end-to-end against a real local `postgres` server, not just mocked tests.
- Added an update check and `mcp-db-connect update` command. `start`/`serve-http` check npm once a day (cached, non-blocking, silent on failure) and print an available-update notice to stderr only (stdout stays clean for the stdio transport's JSON-RPC stream). The check — and the new `update` command by default — only considers versions within `^<current version>` (the current `0.x` minor line), not a blind `@latest`, since this project hasn't reached `1.0.0` yet and a minor bump could still be breaking. `update` detects a global vs. local install and runs the matching `npm install` command; there's no silent background auto-install.
- Fixed the pino logger writing to stdout by default: on the stdio transport, that could corrupt the JSON-RPC stream unless the AI client's generated config happened to set `LOG_LEVEL=silent` (which the setup wizard does, but a manually-run `mcp-db-connect start` or a different client config would not have been protected). The logger now always writes to stderr regardless of `LOG_LEVEL`, which still controls verbosity as before. Verified end-to-end that stdout stays empty on both `start` and `serve-http` with no `LOG_LEVEL` set at all.
